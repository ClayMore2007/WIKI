...
| 影响决策的页面缺来源 | 补来源或标记 `needs source` |
| raw 多但无主题页 | 建议新增或更新主题页 |
| Cubox title-only 资料 | 标记内容提取优先级 |
| notes_template TODO | 单列 `_notes_template.md` 中仍有 `TODO:` 的资料，标记为已提取但未编译；若同 ID 已有 `_transcript.md`，列为优先编译项 |
| 提取闭环缺口 | 检查同一 ID 的 metadata、transcript、segments、notes_template、wiki 页面和资料脑图；缺项写入整理报告；历史关系图仅作为迁移检查项 |
